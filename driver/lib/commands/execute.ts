...
const tryExecuteCustom = async (
driver: FlutterDriver,
command: string,
args: any // в старой версии это был Map<string, string>, но на практике приходил обычный объект
): Promise<any> => {
const payload = {
command,
...args
...
try {
return await driver.socket!.executeSocketCommand(payload);
  } catch (err) {
throw new Error(`Error while executing custom command ${command}: ${err}`);
  }
};
